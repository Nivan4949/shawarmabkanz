        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {galleryItems.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-[#1D1D1D] rounded-2xl border border-white/5 overflow-hidden flex flex-col hover:border-[#E61C24]/30 hover:shadow-2xl transition-all duration-300 relative group aspect-square"
              >
                {/* Image backdrop */}
                <img
                  src={item.image}
                ...
                <div className="absolute bottom-6 left-6 right-6 z-10 flex flex-col transition-all duration-300 group-hover:opacity-0 group-hover:translate-y-2">
                  <span className="text-[10px] text-[#F1C40F] font-bold uppercase tracking-wider mb-1">
                    {category}
                  </span>
                ...
                <div className="absolute inset-0 bg-[#0E0E0E]/90 p-6 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                  <div className="w-8 h-8 rounded-full bg-[#E61C24]/10 text-[#E61C24] flex items-center justify-center mb-4">
                    <Eye className="w-4 h-4" />
                  </div>
                  <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold mb-3">
