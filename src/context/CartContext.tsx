...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [deliveryFee, setDeliveryFeeState] = useState(2.99);
  ...
  const applyCoupon = (code: string): boolean => {
    ...
    if (formattedCode === "KANZFEST") {
      ...
      setCouponCode(formattedCode);
      setDiscountPercent(100); // 100% Off
      return true;
  ...
  const placeOrder = (details: {
    ...
    // Save order to localStorage for Admin Panel simulation
    ...
    // Simulate spit roast step after 2s
    setTimeout(() => {
      setSuccessStep(2);
    ...
    // Simulate transit step after 6s
    setTimeout(() => {
      setSuccessStep(3);
      const orders = JSON.parse(localStorage.getItem("kanz_orders") || "[]");
